...
# Todo: remove mongo_connection module when samples_api is completely refactored
...
class API:
    ...
    def get_samples_of_species(self, species_name, filter: dict = None):
        pipeline = list()
        if species_name == 'all':
            match = dict()
        else:
        ...
        if filter:
            for key in filter.keys():
                if key == 'species':
                    raise ValueError("Filtering on species is not allowed here.")
...
def _filter(run_names = None,
    ...
    if species_source == "provided":
        spe_field = FIELD_MAPPING["provided_species"]
    elif species_source == "detected":
        spe_field = FIELD_MAPPING["detected_species"]
    else:
        spe_field = FIELD_MAPPING["species"]
    connection = get_connection(connection_name)
    db = connection.get_database()
    query = []
    sample_set = set()
    ...
    if date_range is not None and len(date_range) == 2:
        ...
        query.append({FIELD_MAPPING["date_sequenced"]: date_range_query})
    ...
    if group is not None and len(group) != 0:
        if "Not defined" in group:
            query.append({"$or":
                [
                    {FIELD_MAPPING["group"]: None},
                    {FIELD_MAPPING["group"]: {"$in": group}},
                    {FIELD_MAPPING["group"]: {
                        "$exists": False}}
                ]
            })
...
def get_group_list(run_name = None, connection_name = "default"):
    ...
    if run_name is not None:
        ...
        groups = list(db.samples.aggregate([
            ...
            {
                "$group": {
                    "_id"  : "${group}".format(**FIELD_MAPPING),
                    "count": {"$sum": 1}
                }
            }
        ]))
